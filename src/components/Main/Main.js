...
const Main = ({cityName}) => {
   ...
   useEffect(() => {
     ...
     !cityName && navigator.geolocation.getCurrentPosition(position => {
   ...
   const fetchData = (place) => {
	     ...
	     .then(axios.spread(( currW, day24Hours, sevenDaysW ) => {
	       currW.status === 200 && setCurrentWeather(currW.data.data[0]);
	       day24Hours.status === 200 && setByHour(day24Hours.data.data);
